...
class Window(QWidget):
    ...
    def result(self):
        ...
        try:
            ...
            try:
                ...
                return
        if self.op == "+": self.line.setText(str(float(self.num1) + float(self.num2)))
        if self.op == "-": self.line.setText(str(float(self.num1) - float(self.num2)))
        if self.op == "*": self.line.setText(str(float(self.num1) * float(self.num2)))
        if self.op == "/":
            try:
    ...
    def keyPressEvent(self, e):
        if e.key() == Qt.Key_Plus: self.operation("+")
        if e.key() == Qt.Key_Minus: self.operation("-")
        if e.key() == Qt.Key_Enter: self.result()
        if e.key() == Qt.Key_Equal: self.result()
        if e.key() == Qt.Key_Slash: self.operation("/")
        if e.key() == Qt.Key_Asterisk: self.operation("*")
        if e.key() == Qt.Key_1: self.addText("1")
        if e.key() == Qt.Key_2: self.addText("2")
        if e.key() == Qt.Key_3: self.addText("3")
        if e.key() == Qt.Key_4: self.addText("4")
        if e.key() == Qt.Key_5: self.addText("5")
        if e.key() == Qt.Key_6: self.addText("6")
        if e.key() == Qt.Key_7: self.addText("7")
        if e.key() == Qt.Key_8: self.addText("8")
        if e.key() == Qt.Key_9: self.addText("9")
        if e.key() == Qt.Key_0: self.addText("0")
        if e.key() == Qt.Key_Period: self.addText(".")
        if e.key() == Qt.Key_Delete: self.clearText()
        if e.key() == Qt.Key_Backspace: self.backspace()
